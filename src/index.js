...
function getDismantling(inputNum) {
  let left = 0, right = 0, length = 0, isMinus = inputNum < 0;
...
export function ride(n1, n2) {
  ...
  const l1 = Math.abs(left1), l2 = Math.abs(left2), r1 = Math.abs(right1), r2 = Math.abs(right2);

  const back = plus(plus(l1 * l2, getFloat(l1 * r2, num)), plus(getFloat(l2 * r1, num), getFloat(r1 * r2, num * num)));
